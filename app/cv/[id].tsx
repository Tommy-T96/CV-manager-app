...
import { Trash2, Edit, Download } from 'lucide-react-native';
        ...
        <CVDetailSection title="Education">
          {selectedCV.education.map((edu, index) => (
            <View key={index} style={styles.educationItem}>
              <Text style={styles.degreeText}>{edu.degree} in {edu.field}</Text>
            ...
            <View style={styles.languagesContainer}>
              {selectedCV.languages.map((language, index) => (
                <Text key={index} style={styles.languageText}>
                  {language}{index < selectedCV.languages!.length - 1 ? ', ' : ''}
                </Text>
              ))}
            </View>
          </CVDetailSection>
