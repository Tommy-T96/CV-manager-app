...
export default function CVListScreen() {
  ...
  const {cvs, searchQuery, searchResults, setSearchQuery, searchCVs, selectCV} =
    useCVStore();
  ...
  const displayedCVs = searchQuery.trim()
    ? searchResults.map(result => result.cv)
    : cvs;
    ...
    <View style={styles.container}>
      ...
      {displayedCVs.length > 0 ? (
        <FlatList
          ...
          renderItem={({item}) => (
            <CVCard cv={item} onSelect={() => handleSelectCV(item.id)} />
          )}
