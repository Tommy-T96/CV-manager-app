...
export const processNaturalLanguageQuery = async (
  ...
  const results: CVSearchResult[] = cvs
    .map(cv => {
      ...
      if (
        matchedPattern.field === 'all' ||
        matchedPattern.field === 'experience'
      ) {
        const expMatches = cv.experience.filter(
          exp =>
      ...
      if (
        matchedPattern.field === 'all' ||
        matchedPattern.field === 'education'
      ) {
        const eduMatches = cv.education.filter(
          edu =>
...
export const generateQueryResponse = async (
  ...
  if (
    lowercaseQuery.includes('who has experience with') ||
    lowercaseQuery.includes('who knows')
  ) {
...
${results
  .map(
    (r, i) =>
      `${i + 1}. ${r.cv.name} - ${
        r.cv.experience[0]?.position || 'No position'
      } at ${r.cv.experience[0]?.company || 'No company'}`,
  )
  .join('\n')}`;
  }
  ...
  if (lowercaseQuery.includes('which cvs mention')) {
    return `I found ${
      results.length
    } CVs mentioning your search term. The most relevant ones are from ${results
      .slice(0, 3)
      .map(r => r.cv.name)
      .join(', ')}.`;
  }
  ...
  // Default response
  return `I found ${results.length} results matching your query. The top matches are from ${results
    .slice(0, 3)
    .map(r => r.cv.name)
    .join(', ')}. You can view their detailed profiles for more information.`;
};